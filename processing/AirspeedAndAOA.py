...
df_dP1.loc[:, 'dP1_bs(Pa)'] = df_dP1.loc[:, 'dP1(Pa)'] - dP1_bg_mean
df_dP2.loc[:, 'dP2_bs(Pa)'] = df_dP2.loc[:, 'dP2(Pa)'] - dP2_bg_mean
...
df_dP2.loc[:, 'time_zero(s)'] = df_dP2['time(s)'] - df_dP2.loc[0, 'time(s)']
...
df_Data.loc[:, 'dP1/dP2'] = df_Data.loc[:, 'dP1(Pa)'].divide(df_Data.loc[:, 'dP2(Pa)'])
pressure_ratio = df_Data.loc[:, 'dP1/dP2'].values
...
s_flight_times_bool = df_Data['time(s)'].between(MIN_TIME, MAX_TIME)
